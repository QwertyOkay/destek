const body=document.querySelector("body");export function getParramUtm(value){let params=new URLSearchParams(document.location.search);return params.get(value);}
...
export const generationsCustomPassword=()=>{const chars="0123456789abcdefghijklmnopqrstuvwxyz!@#$%^&*()ABCDEFGHIJKLMNOPQRSTUVWXYZ";const passwordLength=7;let password="";for(let i=0;i<=passwordLength;i++){const randomNumber=Math.floor(Math.random()*chars.length);password+=chars.substring(randomNumber,randomNumber+1);}
return password;};export function renderFormRegistrations(allFormClass){const forms=document.querySelectorAll(`.${allFormClass}`);const htmlFormHtml=`
 ...
 <div class="form-group input-group input-group--name input-box">
  ...
  <span class="form-error-content">неправильный номер</span>
  ...
  <div class="user-policy-box d-flex px-5 pt-3">
                    ...
                    <p class="user-policy">Я даю согласие на сбор адреса моей электронной почты с целью получения
                        коммерческих предложений, которые, по нашему мнению, будут интересны вам, от имени компаний и
                        отраслей, подробно описанных в наших Условиях пользования и Политике конфиденциальности.</p>
                </div>

      `;forms.forEach((form,index)=>{form.insertAdjacentHTML("beforeend",htmlFormHtml);});}
export function generationsModalErrors(){const html=`
...
export function preloaderFormSend(){const preloader=`
